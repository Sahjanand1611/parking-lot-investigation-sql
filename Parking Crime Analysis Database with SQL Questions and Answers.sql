...
SELECT * FROM parking_log  WHERE check_out IS NOT NULL;
-- Payment data
select p.name, p.license_number,p1.car_plate,p1.check_in,p1.check_out,p2.payment_time,p2.car_plate,s.claimed_hours, s.reason from statements s join persons p on p.person_id=s.person_id join
parking_log p1 on p1.person_id = s.person_id join payments p2 on p2.person_id =s.person_id;

SELECT * from  payments where payment_id is not null;

-- Police statements from people who were present

select p.name, p.license_number,p1.car_plate,p2.payment_time,p2.car_plate,s.claimed_car,s.claimed_hours from statements s join persons p on p.person_id=s.person_id join
parking_log p1 on p1.person_id = s.person_id join payments p2 on p2.person_id =s.person_id where p1.car_plate != p2.car_plate;


select p.log_id,p.car_plate,p1.payment_id,p1.car_plate,p1.payment_time,p2.person_id,p2.name from statements s join parking_log p on p.person_id = s.person_id join payments p1 on p1.person_id =s.person_id join persons p2 on p2.person_id=s.person_id
where p.car_plate != p1.car_plate;
